...
extern "C" {
#endif /* __cplusplus */
extern volatile int _FLASH_IsDisable_ICache;
extern volatile int _FLASH_IsDisable_DCache;
...
__STATIC_FORCEINLINE void FLASH_EnterCritical(void) {
    if (_FLASH_CriticalNesting <= 0) {
        ...
        //    SCB_DisableICache();
        //    _FLASH_IsDisable_ICache = 1;
        //}
        if (SCB_CCR_DC_Msk == (SCB_CCR_DC_Msk & SCB->CCR)) {
            SCB_DisableDCache();
            _FLASH_IsDisable_DCache = 1;
        }
        //ARM_MPU_Disable();
...
__STATIC_FORCEINLINE void FLASH_ExitCritical(void) {
    ...
    if (_FLASH_CriticalNesting <= 0) {
        ...
        _FLASH_CriticalNesting = 0;//允许多次调用exit
        EnableGlobalIRQ(_FLASH_regPrimask);
        //ARM_MPU_Enable(MPU_CTRL_PRIVDEFENA_Msk);
        //if (_FLASH_IsDisable_ICache) {
        //    SCB_EnableICache();
        //    _FLASH_IsDisable_ICache = 0;
        //}
		if (_FLASH_IsDisable_DCache) {
			SCB_EnableDCache();
			_FLASH_IsDisable_DCache = 0;
		}
	}
}
